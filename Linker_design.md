...
1. **Parse Command-Line Arguments:**  
   - The linker will be invoked like:  
     ```bash
     linker main.o math.o -o program.vm
     ```  
   - It will parse this to get a list of input `.o` files and one output `.vm` file name.  

2. **Read and Parse Object Files:**  
...
3. **Merge Sections & Build Global Symbol Table (Symbol Resolution):**  
   - Initialize:  
     ```cpp
   ...
   - Iterate through each `ParsedObjectFile`:  
     - Iterate through its `symbol_table`:  
       - Calculate the symbol's final absolute address:  
         - If **TEXT**:  
           ```
           final_address = current_code_offset + symbol.address
           ```  
         - If **DATA**:  
           ```
           final_address = total_code_size + current_data_offset + symbol.address
           ```  
       - If the symbol is **GLOBAL**:  
         ...
         - Otherwise, add it to the Global Symbol Table with its final address.  
       - If the symbol is **LOCAL**, ignore it for the global table (it won’t be referenced externally).  
     - Update offsets for the next file:  
       ```cpp
       current_code_offset += file.code_section.size();
       current_data_offset += file.data_section.size();
       ```  

4. **Perform Relocation:**  
   - Create two large byte vectors:  
     ```cpp
     final_code_section
     final_data_section
     ```  
   - Initialize `current_code_offset = 0` for tracking.  
   - Iterate through each `ParsedObjectFile`:  
     - Append its `code_section` to `final_code_section`.  
     - Iterate through its `relocation_table`:  
       ...
       - Compute patch location:  
         ```
         patch_location = current_code_offset + reloc_entry.offset
         ```  
       - Write the 4 bytes of `final_address` into `final_code_section` at `patch_location`.  
     - Update `current_code_offset` for the next file.  

5. **Write Executable (`.vm`) File:**  
   - Open the output file for binary writing.  
   - Write the `.vm` header:  
     - Magic Number  
     - Final address of the `main` symbol (looked up from the Global Symbol Table)  
     - Total code size  
     - Total data size  
